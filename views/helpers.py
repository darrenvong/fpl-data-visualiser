...
def connect(on_heroku=False):
    """Connects to the system's MongoDB database.
    @keyword on_heroku: True if the system is deployed on Heroku (for beta testing)
    and thus make use of Heroku's complementary MongoLab database. False otherwise.
    @return a tuple with a reference to the client connection to the database and
    a reference to the current game week's players data collection
    """
